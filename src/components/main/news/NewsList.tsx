...
export default function NewsList() {
  ...
  animationDuration = Math.min(Math.max(animationDuration, minDuration), maxDuration)
    ...
    <div>
      ...
      >
        ...
        <button
          ...
          style={{ transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)' , transitionDuration: `${animationDuration}ms`}}
        >
      ...
      >
        <div className="news-list">
          {news1.map(({ headers, date, description, status }) => (
            <div key={headers} className="news-card rounded-md transition-colors duration-200">
